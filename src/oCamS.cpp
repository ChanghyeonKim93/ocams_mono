...
//std::cout<<left_image.type<<std::endl; 
//cv::cvtColor(dstImg[0],right_image,CV_BayerGR2RGB);
//cv::cvtColor(dstImg[1],left_image ,CV_BayerGR2RGB);
            return true;
        } else {
            return false;
        }
...
private:
    ...
    void device_poll() {
        ...
        info_manager.loadCameraInfo( "package://ocams_mono/config/left.yaml");
        left_info = info_manager.getCameraInfo();

        info_manager.setCameraName("right");
        info_manager.loadCameraInfo( "package://ocams_mono/config/right.yaml");
        ...
        while (ros::ok())
        {
            ...
            /* time stamp publish */
            sec = (uint32_t)(time_stamp/1000+1000000000);
            ...
            if (left_image_pub.getNumSubscribers() > 0) {
                publishImage(left_image, left_image_pub, "left_frame", measurement_time);
            }
            if (right_image_pub.getNumSubscribers() > 0) {
                publishImage(right_image, right_image_pub, "right_frame", measurement_time);
            }
            if (left_cam_info_pub.getNumSubscribers() > 0) {
                publishCamInfo(left_cam_info_pub, left_info, measurement_time);
            }
            if (right_cam_info_pub.getNumSubscribers() > 0) {
                publishCamInfo(right_cam_info_pub, right_info, measurement_time);
            }

//std::cout<<"type:"<<left_image.type()<<std::endl;
//std::cout<<"size:"<<left_image.channels()<<std::endl;
            if (show_image_) {
...
public:
  ...
  void publish_topic()
  {
    ...
    imu_magnetic_msg.header.stamp = measurement_time; // original : now kch
...
int main (int argc, char **argv)
{
    ...
    {
      ROS_ERROR("%s\n", "IMU initialize false!\r\n oCamS-1MGN-U sends IMU data through Virtual COM port.\r\n \
