...
bool Camera::get_current_format(camera_format& fmt)
{
    ...
    std::cout<<"camera pixel format : "<<((fmt.pixformat>>0) & 0xFF) <<","<<((fmt.pixformat>>8) & 0xFF)<<","<< ((fmt.pixformat>>16) & 0xFF) <<","<<((fmt.pixformat>>24) & 0xFF) <<","<<std::endl; // kch

    fmt.width  = v4l2_s.format.fmt.pix.width;
    fmt.height = v4l2_s.format.fmt.pix.height;
    std::cout<<"camera width and height : "<< fmt.width <<" , "<< fmt.height <<std::endl; // kch
